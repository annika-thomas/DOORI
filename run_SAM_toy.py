...
#samModel = SamWrapper(pathToCheckpoint, device, conf, iou)
samModel = SAM('sam_b.pt')
...
inputFolder = '/home/annika/DORI/example_images_in/'
outputFolder = '/home/annika/DORI/example_images_out_SAM/'
...
for filename in files:
    ...
    try:
        ...
        if (os.path.isfile(filepath_in) and (filepath_in.endswith("png") or filepath_in.endswith("jpg"))):
            ...
            # Run FastSAM
            #segmask = samModel.segmentFrame(image)
            DEVICE = 'cuda'
            everything_results = samModel(image, device=DEVICE, retina_masks=True, imgsz=1024, conf=conf, iou=iou,)
